...
def plotSinglePointsToTensor(cam_type:int,cam_data:(torch.Tensor or list[float]), points:torch.Tensor, environment_type:int, environment:torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    module_name=f"plot CAM_TYPE={cam_type} NDIM={ndim} STRUCTURAL_REFINEMENT={int(args.STRUCTURAL_REFINEMENT)} ENVIRONMENT_TYPE={environment_type}{f' ENVIRONMENT_RESOLUTION={environment.size(-2)}' if environment_type==2 else ''} COMPUTE_STABILITY={args.compute_stability} BLEND_MODE={args.blend_mode}"
