...
try:
    ...
    if(args.time_render_speed):
        ...
        print(f"Average time (forward  w/  nn):{rf.mean()}, deviation:{rf.std()} {torch.std_mean(rf)}, median {rf.median()}, min/max {rf.min()}/{rf.max()}")
        print(f"Average time (backward w/  nn):{rb.mean()}, deviation:{rb.std()} {torch.std_mean(rb)}, median {rb.median()}, min/max {rb.min()}/{rb.max()}")
        print(f"Average time (forward  w/o nn):{nf.mean()}, deviation:{nf.std()} {torch.std_mean(nf)}, median {nf.median()}, min/max {nf.min()}/{nf.max()}")
        print(f"Average time (backward w/o nn):{nb.mean()}, deviation:{nb.std()} {torch.std_mean(nb)}, median {nb.median()}, min/max {nb.min()}/{nb.max()}")
